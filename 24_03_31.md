...
```cpp
	...
	bool Move(int idx, int dir) {
		...
		while (true) {
			...
			for (auto& v : tmp_knight) {
				if (v.r == nr && v.c == nc) { // 다른 기사가 있을 경우
					if (!Move(v.index-1, dir)) return false; // 이동 실패한 경우 이동하지 않음
				}
			}
	...
	void Damage() {
		for (auto& [r,c,h,w,k,idx,o] : knight) {
			if (k <= 0) continue; // 죽었으면 넘어가기
			if (o) continue;//명령을받고 움직인 기사면 넘어가기
			int damage = 0;
	...
	int main(void) {
		...
		for (auto& [idx, dir] : Orders) {
			if (Move(idx - 1, dir)) {
				knight[idx - 1].is_ordered = true;
				Damage();
				knight[idx - 1].is_ordered = false;
			}
		}
